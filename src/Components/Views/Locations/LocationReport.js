...
import { GET_LOCATION_TAG_REPORT } from './GraphQL'
import { SET_NAVBAR_PROPERTIES, SET_SIDEBAR_PROPERTIES } from '../../../Redux/Actions'
import emptyImage from '../../../Assets/img/undraw_revenue_3osh.svg'
	...
	const getReport = async () => {
		...
		try {
			const { data } = await client.query({
				query: GET_LOCATION_TAG_REPORT,
				variables: { location, from: start, to: end },
				fetchPolicy: 'network-only',
			})
			if(data && data.Invoice && data.Invoice.locationReport){
				setReports(data.Invoice.locationReport)
			}
		} catch(e){
			console.log(e)
		}
